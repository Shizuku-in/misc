...
def subset_font_task(font_path, font_index, text, output_dir, disable_subset=False):
...
def process_mkv(mkv_path, args, font_manager, temp_dir):
    ...
    with Progress(
        ...
        for font_name, info, chars in valid_fonts:
            ...
            path, mime = subset_font_task(file_path, font_index, chars, temp_dir, args.disable_subset)
            if path:
